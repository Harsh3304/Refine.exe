...
from model import DenoisingCNN, ColourizeCNN  # Import the models
from scipy.ndimage import median_filter  # Import median filter for noise reduction

file_path = os.getcwd()  # Set the file path to current directory

# Load the models
denoise_model = DenoisingCNN()
denoise_model.load_state_dict(torch.load("denoising_cnn_150.pth"))
denoise_model.eval()

colourize_model = ColourizeCNN()
colourize_model.load_state_dict(torch.load("colorization_model_100eph.pth"))
...
# Function to apply median filter and remove salt-and-pepper noise
...
# Function to convert NumPy array to Tkinter-compatible image
...
def denoise_image(image):
    # Apply median filter first to reduce salt-and-pepper noise
    image_filtered = apply_median_filter(image, filter_size=3)
    
    # Convert to tensor and use the denoising model
...
# Function for colorization of grayscale images
...
# Function to open the denoise screen
def denoise_screen():
    ...
    def add_image():
        ...
        if img_path:
            img = Image.open(img_path).convert('RGB')  # RGB input
...
# Function to open the colorize screen
...
def main():
    ...
    # Background and button image paths
